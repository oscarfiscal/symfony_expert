...
class CategoController extends AbstractController
{
    ...
    public function new(CategoryRepository $categoryRepository, PersistenceManagerRegistry $entityManager,Request $request): Response
    {
        ...
        if($this->isCsrfTokenValid('category',$request->request->get('_token'))){
            ...
            if($name && $color){
                ...
                if(!$color){
                    $this->addFlash('danger','El color de la categoria no puede estar vacio');
                }
    ...
    #[Route('/{id}/update', name: 'app_update_category')]
    public function update(Category $category, PersistenceManagerRegistry $entityManager,Request $request): Response
    {
        $category = new Category();
        if($this->isCsrfTokenValid('category',$request->request->get('_token'))){
            $name = $request->request->get('name',null);
        ...
        return $this->render('catego/update.html.twig', [
            'category' => $category,
        ]);
    }

    #[Route('/{id}/delete', name: 'app_delete_category')]

    public function delete(Category $category, PersistenceManagerRegistry $entityManager): Response
    {
        $entityManager = $entityManager->getManager();
        $entityManager->remove($category);
        $entityManager->flush();
        $this->addFlash('success','Categoria eliminada');
        return $this->redirectToRoute('app_catego');
    }
