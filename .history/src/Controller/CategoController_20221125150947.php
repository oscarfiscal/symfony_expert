...
class CategoController extends AbstractController
{
    ...
    public function update(Category $category, CategoryRepository $categoryRepository, PersistenceManagerRegistry $entityManager,Request $request): Response
    {
        $category = new Category();
        if($this->isCsrfTokenValid('category',$request->request->get('_token'))){
            $name = $request->request->get('name',null);
            ...
            if($name && $color){
                ...
                $entityManager->persist($category);
                $entityManager->flush();
                $this->addFlash('success','Categoria actua');
                return $this->redirectToRoute('app_catego');
            }else{
                if(!$name){
                    $this->addFlash('danger','El nombre de la categoria no puede estar vacio');
