...
function edit(Request $request, Marker $marker, MarkerRepository $markerRepository, PersistenceManagerRegistry $doctrine): Response
    ...
    if ($form->isSubmitted()) {
        if ($form->isValid()) {
            ...
            foreach ($markerTagsCurrents as $markerTags) {
                $delete= true;
                ...
                if ($delete) {
                    $marker->remove($markerTags);
                }
                foreach ($tags as $tag) {
                    $created = true;
                    foreach ($markerTagsCurrent as $markerTags) {
                        if ($tagCurrent->getId() == $tag->getId()) {
                            $created = false;
                        }
                    }
                    if ($created) {
                        $markerTags = new MarkerTag();
                        $markerTags->setMarker($marker);
                        $markerTags->setTag($tag);
                        $entityManager->persist($markerTags);
                    }
                }
