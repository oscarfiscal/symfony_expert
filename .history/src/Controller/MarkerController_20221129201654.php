...
function edit(Request $request, Marker $marker, MarkerRepository $markerRepository, PersistenceManagerRegistry $doctrine): Response
    ...
    $markerTagsCurrents = $marker->getMarkerTags();
    if ($form->isSubmitted()) {
        if ($form->isValid()) {
            $entityManager = $doctrine->getManager();
            $tags = $form->get('tag')->getData();

            foreach ($markerTagsCurrents as $markerTagsCurrent) {
                $delete= true;
                ...
                if ($delete) {
                    $marker->removeMarkerTag($markerTagsCurrent);
                }
                foreach ($tags as $tag) {
                    $created = true;
                    foreach ($markerTagsCurrent as $markerTagsCurrent) {
                        if ($tagCurrent->getId() == $tag->getId()) {
                            $created = false;
                        }
                    }
                    if ($created) {
                        $markerTagsCurrent = new MarkerTag();
                        $markerTagsCurrent->setMarker($marker);
                        $markerTagsCurrent->setTag($tag);
                        $entityManager->persist($markerTagsCurrent);
                    }
                }
