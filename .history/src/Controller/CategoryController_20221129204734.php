...
#[Route('/{category}/{page}', name:'app_category', defaults:["category" => "todas", "page" => 1], requirements:["page" => "\d+"], methods:["DELETE"])]
function index(string $category, int $page, CategoryRepository $categoryReposotiry, MarkerRepository $markerRepository): Response
    {
    ...
    if ($category && 'todas' !== $category) {
        ...
        if (!$categoryReposotiry->findByName($category)) {
            throw $this->createNotFoundException(" las categoria '$category' no existe");
        }
