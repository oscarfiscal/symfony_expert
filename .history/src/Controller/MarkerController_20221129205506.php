...
function edit(Request $request, Marker $marker, MarkerRepository $markerRepository, PersistenceManagerRegistry $doctrine): Response
    ...
    if ($form->isSubmitted()) {
        if ($form->isValid()) {
            ...
            foreach ($markerTagsCurrents as $markerTagsCurrent) {
                ...
                foreach ($tags as $tag) {
                    ...
                    if ($created) {
                        $markerNew = new MarkerTag();
                        $markerNew->setMarker($marker);
                        $markerNew->setTag($tag);
                        $entityManager->persist($markerNew);
                    }
                }
        ...
        $tags = [];
        foreach ($markerTagsCurrents as $markerTag) {
...
#[Route('/{id}/delete', name:'app_marker_delete', methods:['DELETE'])]
